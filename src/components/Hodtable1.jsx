...
function Hodtable1(){
    ...
    useEffect(()=>{
        inst.get('api/aobookings/').then(response=>{setData(response.data)
    ...
    const handleSubmit1=(item)=>{
      console.log(item)
      const response=  inst.put(`api/ao/${item.id}/`,{ao_remark:`${item.remark}`,
      ao_approval_status	: '1'
